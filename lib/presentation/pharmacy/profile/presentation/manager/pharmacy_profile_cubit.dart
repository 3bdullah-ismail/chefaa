...
import '../../data/repositories/pharmacy_profile_repo.dart';
...
class PharmacyProfileCubit extends Cubit<PharmacyProfileState> {
  ...
  Future<void> getPharmacyProfile() async {
    ...
    try {
      ...
      emit(PharmacyProfileSuccess(response));
    } catch (e) {
      emit(PharmacyProfileError(e.toString()));
    }
  }

  Future<void> updatePharmacyProfile(Map<String, dynamic> body) async {
    emit(PharmacyProfileUpdateLoading());
    try {
      final response = await pharmacyProfileRepo.updatePharmacyProfile(body);
      emit(PharmacyProfileUpdateSuccess(response));
    } catch (e) {
      emit(PharmacyProfileUpdateError(e.toString()));
    }
  }
}
